...
if (isServer) then 
{
	...
	HVT1 = _grp0 createUnit ["I_BSO_chdkz_COM_A",(getMarkerPos "m_1"),[],250,"FORM"];
	removeAllWeapons HVT1;

	[HVT1, getPos HVT1, 50, 1, true] call CBA_fnc_taskDefend;
	
	grp1 = [getMarkerPos "m_1", resistance, (configfile >> "CfgGroups" >> "Indep" >> "bso_chdkz" >> "Infantry" >> "BSO_chdkz_groupD")] call BIS_fnc_spawnGroup;
	...
	trg1 setTriggerStatements ["!alive hvt1", "[""Task_e_hvt"",""succeeded""] call SHK_Taskmaster_upd;",""];	
...
//};
	...
	["Task_e_hvt","Eliminate the HVT","We have identified the location of one of the Warlords. Move in and eliminate him."] call SHK_Taskmaster_add;
